...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Add New Research Paper</h6>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.papers.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="authors" name="authors" value="{{ old('authors') }}" required placeholder="e.g. John Doe, Jane Smith">
                </div>
                
                <div class="form-group">
                    <label for="published_date">Publication Date <span class="text-danger">*</span></label>
                    <input type="date" class="form-control" id="published_date" name="published_date" value="{{ old('published_date') }}" required>
                </div>
                    ...
                    <div class="categories-checkbox">
                        @foreach($categories as $category)
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input" id="category-{{ $category->id }}" name="categories[]" value="{{ $category->id }}" {{ (is_array(old('categories')) && in_array($category->id, old('categories'))) ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="keywords" name="keywords" value="{{ old('keywords') }}" placeholder="e.g. automation, AI, machine learning">
                    <small class="form-text text-muted">Separate keywords with commas</small>
                </div>
                
                <div class="form-group">
                    <label for="excerpt">Excerpt (Optional)</label>
                    <textarea class="form-control" id="excerpt" name="excerpt" rows="4">{{ old('excerpt') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="references" name="references" rows="4">{{ old('references') }}</textarea>
                    <small class="form-text text-muted">Supports HTML formatting</small>
                </div>
                
                <div class="form-group">
                    <label for="author_bios">Author Biographies (Optional)</label>
                    <textarea class="form-control" id="author_bios" name="author_bios" rows="4">{{ old('author_bios') }}</textarea>
                    <small class="form-text text-muted">Supports HTML formatting</small>
                </div>
                
                <div class="form-group">
                    <label for="paper_file">Paper File <span class="text-danger">*</span></label>
                    <input type="file" class="form-control-file" id="paper_file" name="paper_file" accept=".pdf,.doc,.docx" required>
